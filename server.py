        ...
        try:
            ...
            for table, query in sources:
                ...
                for r in rows:
                    ...
                    auto_cat = self.CATEGORY_MAP.get('', '')
                    ...
                    for k in ('created_at', 'upload_time', 'date', 'record_date'):
                        try:
                            ...
                            pass
                    conn.execute('''INSERT INTO doc_index (source_table, source_id, club_name, file_name, file_path, description, tags, category, status, source_type, created_at, indexed_at)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime("now","localtime"))''',
                        (table, r['id'], cn, fname, fpath, (desc or '')[:500], tags, auto_cat or cat, r['status'] or '', table, time_val))
                    total += 1
            conn.commit()
            return {'success': True, 'indexed': total}
        finally:
            conn.close()
...
- 用中文回答'''
    ...
    try:
        conn = db.get_conn()
        conn.execute('INSERT INTO ai_chat_history (user_id, role, content) VALUES (?, ?, ?)', (user['id'], 'assistant', reply))
        conn.commit()
        conn.close()
    except:
        pass
...
def chat_history():
    ...
    conn = db.get_conn()
    try:
        rows = conn.execute('SELECT role, content, created_at FROM ai_chat_history WHERE user_id=? ORDER BY created_at ASC LIMIT 100', (user['id'],)).fetchall()
